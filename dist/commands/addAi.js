...
async function addAi() {
    console.log(`${chalk.cyan.bold("Adding Langchain AI features to your project...")}`);
    ...
    const commonTemplatePath = join(commonTemplateDir, langDir, commonAiSetupFile);
            ...
            const { overwriteCommon } = await inquirer.prompt([
                {
                    ...
                    message: `${chalk.yellow("⚠️")} ${commonAiSetupFile} already exists in src/ai/common/. Overwrite?`,
                    default: false,
                },
            ]);
            if (!overwriteCommon) {
    ...
    const modelExampleFile = `${modelChoice.value}-example.${isTypeScript ? "ts" : "js"}`;
    const modelTemplatePath = join(modelSpecificTemplateDir, langDir, modelExampleFile);
    ...
    if (existsSync(modelTemplatePath)) {
        ...
        modelFileContent = modelFileContent.replace(/import \{ translateText, askLLM \} from "\.\.\/\.\.\/common\/.*\/ai-setup\.js";/, `import { translateText, askLLM } from "../common/ai-setup.js";`);
        if (existsSync(modelTargetPath)) {
            const { overwriteModel } = await inquirer.prompt([
                {
                    type: "confirm",
                    name: "overwriteModel",
                    message: `${chalk.yellow("⚠️")} ${modelExampleFile} already exists in src/ai/${modelChoice.value}/. Overwrite?`,
                    default: false,
                },
            ]);
            if (!overwriteModel) {
        ...
        console.log(`${chalk.yellow(`No specific example template found for ${modelChoice.name}. Skipping model-specific example.`)}`);
    }
    const depsToInstall = [
        "langchain",
        "@langchain/core",
    ...
    console.log(`\n${chalk.bgGreen.black(" SUCCESS ")} ${chalk.green.bold(`Langchain with ${modelChoice.name} added successfully!`)} ${chalk.green("🚀")}`);
    console.log(`\n${chalk.cyan.bold("Next steps:")}`);
    if (existsSync(commonTargetPath)) {
        console.log(`  - Explore the common setup in ${chalk.yellow(`src/ai/common/${commonAiSetupFile}`)}.`);
    }
    if (existsSync(modelTargetPath)) {
        console.log(`  - Check out the example in ${chalk.yellow(`src/ai/${modelChoice.value}/${modelExampleFile}`)}.`);
    ...
    if (modelChoice.apiKeyEnv) {
        const apiKeyMessage = modelChoice.value === "google-vertexai"
            ? `Make sure your ${chalk.yellow(modelChoice.apiKeyEnv)} file is correctly set up.`
            : `Make sure to set your ${chalk.yellow(modelChoice.apiKeyEnv)} environment variable. (e.g., create a .env file with ${modelChoice.apiKeyEnv}=your_key_here)`;
