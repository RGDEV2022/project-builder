...
async function addAiRag() {
  console.log(
    `${chalk.cyan.bold("Adding Langchain RAG features to your project...")}`
  );
  ...
  const embeddingChoice = availableEmbeddings.find(
    (e) => e.value === selectedEmbedding
  );
  if (!embeddingChoice) {
    console.error(
      `${chalk.bgRed.white(" ERROR ")} ${chalk.red(
        "Invalid embedding model selection."
      )}`
    );
    process.exit(1);
  }
  ...
  const vectorStoreChoice = availableVectorStores.find(
    (vs) => vs.value === selectedVectorStore
  );
  if (!vectorStoreChoice) {
    console.error(
      `${chalk.bgRed.white(" ERROR ")} ${chalk.red(
        "Invalid vector store selection."
      )}`
    );
    process.exit(1);
  }
  ...
  console.log(
    `${chalk.cyan(
      `Setting up RAG with ${modelChoice.name}, ${embeddingChoice.name} embeddings, and ${vectorStoreChoice.name}...`
    )}`
  );
  ...
  let ragExampleContent = createRagExample(
    isTypeScript,
    modelChoice,
    embeddingChoice,
    vectorStoreChoice
    ...
    const { overwriteRag } = await inquirer.prompt([
      {
        ...
        message: `${chalk.yellow(
          "⚠️"
        )} ${ragExampleFile} already exists in src/ai/rag/. Overwrite?`,
        default: false,
      },
    ]);
    if (!overwriteRag) {
  ...
  console.log(
    `\n${chalk.cyan(
      "Installing Langchain RAG dependencies:"
    )} ${uniqueDeps.join(", ")}`
  );
  ...
  console.log(
    `\n${chalk.bgGreen.black(" SUCCESS ")} ${chalk.green.bold(
      `Langchain RAG with ${modelChoice.name}, ${embeddingChoice.name} embeddings, and ${vectorStoreChoice.name} added successfully!`
    )} ${chalk.green("🚀")}`
  );
  ...
  if (existsSync(ragExamplePath)) {
    console.log(
      `  - Explore the RAG example in ${chalk.yellow(
        `src/ai/rag/${ragExampleFile}`
      )}.`
    );
    console.log(
  ...
  if (vectorStoreChoice.requiresApiKey && vectorStoreChoice.apiKeyEnv) {
    ...
    if (vectorStoreChoice.additionalEnvVars) {
      vectorStoreChoice.additionalEnvVars.forEach((env) =>
  ...
  if (requiredEnvVars.size > 0) {
    ...
    requiredEnvVars.forEach((envVar) => {
      console.log(
        `  - ${chalk.yellow(envVar)}=${
          envVar.includes("KEY") ? "your-api-key" : "your-value"
        }`
      );
    });
    console.log(
      `  Create a ${chalk.yellow(
        ".env"
      )} file with these variables in your project root.`
    );
  }
}

function createRagExample(
  isTypeScript: boolean,
  modelChoice: any,
  embeddingChoice: any,
  vectorStoreChoice: any
) {
...
function createImports(
  ...
  const imports = [
    `import { ${getModelClassName(modelChoice)} } from "${
      modelChoice.packageName
    }";`,
  ...
  switch (vectorStoreChoice.value) {
    case "memory":
      imports.push(
        `import { ${vectorStoreChoice.className} } from "${vectorStoreChoice.importPath}";`
      );
      break;
    case "pinecone":
...
function getModelClassName(modelChoice: any) {
...
function createApiKeyChecks(
  ...
  if (modelChoice.apiKeyEnv) {
    checks.push(`
  const ${modelChoice.apiKeyEnv.toLowerCase()} = process.env.${
  ...
  if (
    embeddingChoice.apiKeyEnv &&
    embeddingChoice.apiKeyEnv !== modelChoice.apiKeyEnv
  ) {
    checks.push(`
  const ${embeddingChoice.apiKeyEnv.toLowerCase()} = process.env.${
      embeddingChoice.apiKeyEnv
    };
  if (!${embeddingChoice.apiKeyEnv.toLowerCase()}) {
    ...
    if (embeddingChoice.additionalEnvVars) {
      embeddingChoice.additionalEnvVars.forEach((envVar: string) => {
        checks.push(`
  ...
  if (vectorStoreChoice.requiresApiKey && vectorStoreChoice.apiKeyEnv) {
    checks.push(`
  const ${vectorStoreChoice.apiKeyEnv.toLowerCase()} = process.env.${
      vectorStoreChoice.apiKeyEnv
    };
  if (!${vectorStoreChoice.apiKeyEnv.toLowerCase()}) {
    ...
    if (vectorStoreChoice.additionalEnvVars) {
      vectorStoreChoice.additionalEnvVars.forEach((envVar: string) => {
        checks.push(`
  const ${envVar.toLowerCase()} = process.env.${envVar};
  if (!${envVar.toLowerCase()}) {
    console.error("${envVar} not found. Please set it in your .env file.");
...
function createLlmInitialization(modelChoice: any) {
...
function createEmbeddingsInitialization(embeddingChoice: any) {
...
function createVectorStoreInitialization(vectorStoreChoice: any) {
  switch (vectorStoreChoice.value) {
    case "memory":
