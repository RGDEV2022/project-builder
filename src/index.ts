...
import { init, questions, Options as InitOptions } from "./commands/init.js"; // .js extension for ESM
  ...
  .action(async (projectName) => {
    try {
      ...
      if (projectName) {
        ...
        options = (await inquirer.prompt(questions as any)) as InitOptions;
      }
      await init(options);
    } catch (error: any) {
      ...
      if (lowerFeature === "express") {
        ...
        console.error(
          `${chalk.bgRed.white(" ERROR ")} ${chalk.red(
            `Unknown feature '${feature}'. Available features: express, ai, ai-rag`
          )}`
        );
        process.exit(1);
      }
